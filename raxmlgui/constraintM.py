#!/usr/bin/env python
# raxmlGUI 0.93 build 20100901. A graphical front-end for RAxML.
# Created by Daniele Silvestro on 21/08/2010. => dsilvestro@senckenberg.de
import Tkinter
import csv
import os
import platform
 ...
 the first name in the list will be selected as outgroup!", parent=master)
    ...
    def add_con():
        ...
        if clade == "()":
            ...
            taxset = taxset + 1
            if taxset == 1:
                label = "1 constraint enforced"
            else:
                label = "%s constraints enforced" % taxset
    ...
    if out_con == 'constraint':
        master.title("Taxa constraint")
    else:
        master.title("Multiple outgroup")
    ...
    for item in range(2, len(taxaL)):
        listbox1.insert(END, taxaL[item])

    if out_con == 'constraint':
        label = ' Constrained taxa'
    else:
        label = ' Outgroup'
    ...
    if out_con == 'constraint':
        ...
        export = StringVar()
        setD = Checkbutton(master, text='Export constraint...', variable=export, onvalue="yes",
                           offvalue="no")  # ALIGNMENT FILE
        setD.grid(row=3, column=3, padx=2, columnspan=1, sticky=W)
        setD.config(bg="light Grey", state=NORMAL)
        setD.deselect()

        constr = StringVar()
        setE = Checkbutton(master, text='Monophyletic contraints', variable=constr, onvalue="yes",
                           offvalue="no")  # ALIGNMENT FILE

...
def export():
	try:
		...
		if ndata > 1:
			ans=askyesno("Warning", "By saving and reloading the alignment, the partition settings will be deleted.\n\nDo you want to proceed? ", parent=self)
			if ans: svname = asksaveasfilename(title="Save and reload alignment", parent=self)
			else: raise
		else: svname = asksaveasfilename(title="Save and reload alignment", parent=self)
		if svname:
...
def export_part():
	...
	partition.export()
def edit():
	showinfo("Notice", "To load the new alignment in raxmlGUI after editing, export it from the menu File: Save alignment", parent=self)
	texxt.config(state=NORMAL)
def parse_data():
...
def mod_tools():
	...
	try: status= "Sequence file > %s  (%s taxa, %s characters, %s)" % (shortpath, numtaxa, numchar, dtype)
	except(NameError): status= "Sequence file > %s  (Fasta file, %s)" % (shortpath, dtype)
	try: frame4.config(bg=col)
	except: update_bar(status)
...
def AAcombi():
	...
	def setAAcombi():
		global dtypeP, ndata
		if data_typeAA.get() in ['GTR', 'GTR_UNLINKED']: dtypeP = "%s" % (data_typeAA.get())
		else: dtypeP = "%s%s" % (data_typeAA.get(), Imodel.get())
		auto_part()
		button2.config(state=NORMAL)
		AAcomb.destroy()
	...
	def def_close_setB_short(a): setAAcombi()
...
#	print part_L
	...
	if dtype != dtypeSafe:
		...
		mod_tools()
	shortpath = os.path.basename(fileinputS)
	status= "Sequence file > %scombi.phy  (%s taxa, %s characters, %s partitions)" % (shortpath, numtaxa, numcharC, ndata)
	statusSafe=status
	update_bar(status)
...
def open_ali(open_arg):
	...
	if	fileinputS:
		try:
			...
			try: opt5.config(state=NORMAL)
			except: pass
		except:
			showerror("Warning", """File format not supported!\n\nThe alignment file must be in "Phylip" format.""", parent=self)
			delete()
def exc_2():
	try:
		...
		frame3.config(bg=col)
		if ndata > 1:
			excyes=askyesno("Warning", "By excluding sites, the partition settings will be deleted. \n\nDo you want to proceed?", parent=self)
			if excyes:
				del_partition()
				exc_3()
...
def exc_3():
	try:
		if dtype=="MIXED":
			...
			import exclude
			exclude.exc_start()
	except(NameError, TclError): showinfo("Warning", "Please load an alignment first!", parent=self)
...
def constraint(v):
 ...
 Do you want to keep the starting tree and run a thorough bootstrap analysis instead?", parent=self)
		...
		#		global const_f
				...
				shortpathconstr = os.path.basename(tree_file)
				shortpath = os.path.basename(fileinputS)
				status= "Sequence file > %s (%s taxa, %s characters) |  Starting tree > %s" % (shortpath, numtaxa, numchar, shortpathconstr)
				update_bar(status)
		...
		if v=='5':
			...
			const_f ="-g %s" % tree_file
			shortpath = os.path.basename(fileinputS)
			status= "Sequence file > %s (%s taxa, %s characters) | Tree constrain enforced" % (shortpath, numtaxa, numchar)
			update_bar(status)
		if v=='6':
			import constraintM
	...
	try:
		...
		if dtype =='DNA':
			...
			if	fileinput:
				...
				IIstr = StringVar()
				opt4 = OptionMenu(frame5, IIstr, 'S6A', 'S6B', 'S6C', 'S6D', 'S6E', 'S7A', 'S7B', 'S7C', 'S7D', 'S7E', 'S7F', 'S16', 'S16A', 'S16B')
				IIstr.set('S16')
				opt4.pack(side=LEFT)
		...
		#		frame2.config(bg=col)
				...
				status= "Sequence file > %s (%s taxa, %s characters) | Secondary str. > %s" % (shortpath, numtaxa, numchar, shortpathpart)
				update_bar(status)
		#		Label(frame2, text=status, bg = col, fg= 'dark blue').pack(side=LEFT)

		else: showinfo("Warning", "Data type has to be DNA to use secondary structure.", parent=self)
	except: showinfo("No dataset ", "Please load an alignment first!", parent=self)
		...
		if	tree_file:
			...
			if dtype == "AA":
				if modelAA.get()== 'GTR' and Imodel.get()== 'F':
					showinfo("Warning", "With amino acid substitution model 'GTR' empirical frequencies cannot be used! Please uncheck the box or choose another substitution model.", parent=self)
					raise(SyntaxError)
				else: mod = "-m %s%s%s" % (model.get(), modelAA.get(), Imodel.get())
			if dtype == "MIXED":
				if part_f=="":
...
"%s" already exist.\n\nChange output name to run the analysis!""" % outfile.get()
	...
	#			showerror("Warning", warn, parent=self)
		...
		if o == "<none>": o = ""
		elif o == "multiple": o = "-o %s" % out_list
		else: o = "-o %s" % o
		...
		if K[5] == "yes":
			...
			if current_OS == 'Windows': result = """&&cd "%s"&& start %s """% (path_dirsimple, result)
			else: result = """&&cd "%s"&& open "%s" """% (path_dirsimple, result)
			...
			if current_OS == 'Windows': result2 = """ &&cd "%s" && start %s """ % (path_dirsimple, bestTree)
			else: result2 = """ &&cd "%s"&& open %s """ % (path_dirsimple, bestTree)
		...
		if "run_CIPRES" in argument:
			...
			elif replicate.get() == 'RELL bootstraps': pass  ## no docs on this yet
...
def open_rooted_tree():
	...
	if rooted_tree:
		...
		try: button_load_tree.config(highlightbackground = col, text='Change tree file', pady=2, command=open_rooted_tree)
		except: pass
...
def import_fasta():
		...
		try:
			...
			if fileinput:
				if 2>1: #try:
					...
					try:
						Matrix=dendropy.DnaCharacterMatrix.get_from_path(fileinput, 'fasta', preserve_underscores=True, data_type='protein', taxon_set=taxa)
						dtype="AA"
					except:
						try:
							Matrix=dendropy.DnaCharacterMatrix.get_from_path(fileinput, 'fasta', preserve_underscores=True, data_type='dna', taxon_set=taxa)
							dtype= "DNA"
	...
	except(ImportError): install_dendropy()
	except: pass
	...
	try:
		...
		if arg=='nex2phy':
			...
			if fileinput:
				try:
					print fileinput
					nex=dendropy.DataSet.get_from_path(fileinput, 'nexus', preserve_underscores=True, suppress_annotations=True)
					#print nex
		...
		if arg=='phy2nex':
			try:
				global ndata, dtype
				print dtype
				if dtype=='DNA': dtypeN='dna'
				elif dtype=='AA': dtypeN='protein'
				else: dtypeN='standard'
				frame3.config(bg=col)
				...
				if svname:
					...
					sv_file.close()
					print dtypeN
					try: ali=dendropy.DataSet.get_from_path(svname, 'phylip', data_type=dtypeN,  preserve_underscores=True, interleaved=True)
					except: ali=dendropy.DataSet.get_from_path(svname, 'fasta', data_type=dtypeN,  preserve_underscores=True, interleaved=True)
					ali.write_to_path(svname, 'nexus', data_type=dtypeN, preserve_underscores=True, simple=True, unquoted_underscores=True)
				else: pass
			except: pass
...
def restart_me():
	...
	elif current_OS=='MacOS': cmdScript= """osascript -e 'tell application "Terminal" to do script "cd '%s' && cd .. && python raxmlGUI.py > output.txt" '""" % self_path
	else: cmdScript= """xterm -e 'cd "%s" && cd .. && python raxmlGUI.py ' & """ % self_path
	print cmdScript
	os.system(cmdScript)
	self.quit()
...
def install_dendropy():
	def select_dendropy_dir():
		...
		if dir:
			...
			else: cmd = """cd "%s" && python setup.py install""" % (dendro_path)
			print cmd

			os.system(cmd)
			ans=askyesno("dendropy", "You must restart raxmlGUI to make the changes effective. Restart now?", parent=self)
			if ans: restart_me()
...
After pressing OK, select the unzipped 'dendropy' folder to proceed with the installation.""", parent=self)
		...
		ans2=askyesno("Library dendropy required", """Install 'dendropy' from local directory?""", parent=self, default='no')
		if ans2:
			select_dendropy_dir()
...
if current_OS == 'MacOS': self.bind("<Command-r>", run_short)
else: self.bind("<Control-r>", run_short)

def pref_short(a): set()
if current_OS == 'MacOS': self.bind("<Command-,>", pref_short)
else: self.bind("<Control-Shift-P>", pref_short)

def del_short(a): delete()
if current_OS == 'MacOS': self.bind("<Command-Shift-K>", del_short)
else: self.bind("<Control-Shift-K>", del_short)

def part_short(a): set_partition()
if current_OS == 'MacOS': self.bind("<Command-p>", part_short)
else: self.bind("<Control-p>", part_short)

def exc_short(a):
	try: exc_2()
	except: pass
if current_OS == 'MacOS': self.bind("<Command-e>", exc_short)
else: self.bind("<Control-e>", exc_short)

def help1_short(a):
	try: help_module(2)
	except: pass
if current_OS == 'MacOS': self.bind("<Command-?>", help1_short)
else: self.bind("<Control-h>", help1_short)

def exit(a): self.destroy
if current_OS == 'MacOS': self.bind("<Command-q>", exit)
else: self.bind("<Control-q>", exit)

def export_ali(a):
	try: export()
	except: pass
if current_OS == 'MacOS': self.bind("<Command-s>", export_ali)
else: self.bind("<Control-s>", export_ali)

def import_F_short(a):
	try: import_fasta()
	except: pass
if current_OS == 'MacOS': self.bind("<Command-f>", import_F_short)
else: self.bind("<Control-f>", import_F_short)

def convert_matrix_short(a):
	try: convert_matrix('nex2phy')
	except: pass
if current_OS == 'MacOS': self.bind("<Command-n>", convert_matrix_short)
else: self.bind("<Control-n>", convert_matrix_short)
...
if current_OS=='MacOS':
	if weird_menu==1: editmenu.add_command(label='Preferences...', command=set) #, accelerator="Command-,")
	else: editmenu.add_command(label='Preferences...', command=set, accelerator="Command-,")
else: editmenu.add_command(label='Preferences...', command=set, accelerator="Ctrl+Shift+P")
...
if current_OS=='MacOS':
	if weird_menu==1: editmenu.add_command(label='Clear', command=delete) #, accelerator="Command-Shift-K")
	else: editmenu.add_command(label='Clear', command=delete, accelerator="Command-Shift-K")
else: editmenu.add_command(label='Clear', command=delete, accelerator="Ctrl+Shift+K")
editmenu.add_separator()
if current_OS=='MacOS':
		if weird_menu==1: editmenu.add_command(label='Quit', command=self.destroy) #, accelerator="Q")
		else: editmenu.add_command(label='Quit', command=self.destroy, accelerator="Command-Q")
else: editmenu.add_command(label='Quit', command=self.destroy, accelerator="Ctrl+Q")

menu.add_cascade(label="File", menu=editmenu)

optmenu = Menu(self) # OPTIONS
optmenu.add_command(label='Select multiple outgroup...', command=lambda: constraint('6'), state=DISABLED)			# EXCLUDE SITES
if current_OS=='MacOS':
	if weird_menu==1: optmenu.add_command(label='Exclude sites...', command=exc_2,state=DISABLED) #, accelerator="E")
	else: optmenu.add_command(label='Exclude sites...', command=exc_2,state=DISABLED, accelerator="Command-E")
else: optmenu.add_command(label='Exclude sites...', command=exc_2,state=DISABLED, accelerator="Ctrl+E")
if current_OS=='MacOS':
	if weird_menu==1: optmenu.add_command(label='Set/Edit partitions...', command=set_partition,state=DISABLED) #, accelerator="P")
	else: optmenu.add_command(label='Set/Edit partitions...', command=set_partition,state=DISABLED, accelerator="Command-P")
else: optmenu.add_command(label='Set/Edit partitions...', command=set_partition,state=DISABLED, accelerator="Ctrl+P")
...
if current_OS=='MacOS':
	if weird_menu==1: optmenu.add_command(label='Save memory search (-F)', command=use_F,state=DISABLED) #, accelerator="F")
	else: optmenu.add_command(label='Save memory search (-F)', command=use_F,state=DISABLED)
else: optmenu.add_command(label='Save memory search (-F)', command=use_F,state=DISABLED)
optmenu.add_separator()
...
if current_OS=='MacOS':
	if weird_menu==1: utimenu.add_command(label='Save and reload alignment', command=export,state=DISABLED)
	else: utimenu.add_command(label='Save and reload alignment', command=export,accelerator="Command-S",state=DISABLED)	#
else: utimenu.add_command(label='Save and reload alignment', command=export,accelerator="Ctrl+S",state=DISABLED)
...
if current_OS=='MacOS':
	if weird_menu==1: helpmenu.add_command(label='Help raxmlGUI', command=lambda: help_module(2)) #, accelerator="?")
	else: helpmenu.add_command(label='Help raxmlGUI', command=lambda: help_module(2), accelerator="Command-?")
else: helpmenu.add_command(label='Help raxmlGUI', command=lambda: help_module(2), accelerator="Ctrl+H")
...
def def_ST(a):
	global const_f
	if a=="User defined": constraint('3')
	else: const_f==''
...
def def_analysis(value):
	...
	if value == 'Ancestral states':
		...
		button_load_tree.config(highlightbackground = col, text='Load tree file', pady=2, command=lambda: open_rooted_tree())
		try: frame3.destroy()
		except: pass
		button5.config(state=DISABLED)
		#__ button7.config(state=DISABLED)
		...
		except: opt5.config(state=DISABLED)
		#button5.config(state=DISABLED)
		#button6.config(state=DISABLED)
...
if current_OS == 'MacOS': self.bind("<Command-a>", a_short)
else: self.bind("<Control-a>", a_short)

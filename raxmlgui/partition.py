...
def part():
    def destroy_w(ans):
        import main
        if ans == 'n':
            main.update_menu('part_on')
        else:
            main.update_menu('set_part')
        self.destroy()
    ...
    def end():
        ...
        ans = askokcancel("Partitioning completed",
                          "All characters have been assigned to partitions.\nSet partitions for the analysis?",
                          parent=self)
        if ans:
            set_part()
        else:
        ...
        try:
            ...
            def model(value, args=None):
                ...
                if data_type.get() == 'AA':
                    ...
                    opt4 = OptionMenu(toolM, data_typeAA, 'DAYHOFF', 'DCMUT', 'JTT', 'MTREV', 'WAG', 'RTREV', 'CPREV',
                                      'VT', 'BLOSUM62', 'MTMAM', 'LG', 'GTR', 'MTART', 'MTZOA', 'PMB', 'HIVB', 'HIVW',
                                      'JTTDCMUT', 'FLU')
            ...
            err = "Illegal entry! Please enter an integer number to define a partition. The number must be in range %s-%s" % (
                from_n.get(), maxchar)
    ...
    def reset_close2():
        ans = askyesno('Warning',
                       "The partition settings will be deleted after closing the window. \nDo you wish to continue?",
                       parent=self)
        if ans:
            import main
            main.del_partition()
    ...
    def reset_close():
        ans = askyesno('Warning',
                       "The partition settings will be deletedafter closing the window. \nDo you wish to continue?",
                       parent=self)
        if ans:
            undo_part()
            import main
    ...
    def edit_part():
        ...
        showinfo("Manual partitioning",
                 "You can now manually edit the partition settings. RAxML will use the exact text you enter in the field.",
                 parent=self)
        texxt.focus_set()

        def def_part_short(a):
            set_part()
    ...
    if platform.system() == "Darwin" or platform.system() == "Windows":
        texxt.config(relief=GROOVE, bg='#e6e6e6', fg='dark blue', font=(11), state=DISABLED)
    else:
        texxt.config(relief=GROOVE, bg='#e6e6e6', fg='dark blue', font=('Monaco', 10), state=DISABLED)

...
def exc_panel():
	...
	def erase_ID():
		...
		else: rm_conf_file = """cd '%s' && rm '%s' """ % (home,conf_file)
		
		print "CMD:",rm_conf_file
		ans = askyesno('Warning', """This will permanently remove the 'pycipres.conf' file and associated information\n\nDo you want to continue?""", parent=self)
...
Note, that anyone could run a job on CIPRES with your login while it is there. To remove permanently this information select the button 'Erase login data'.""", parent=self)
		home = expanduser("~")
		conf_file="%s/pycipres.conf" % home
...
USERNAME=%s
PASSWORD=%s""" % (from_n.get(),to_n.get())
